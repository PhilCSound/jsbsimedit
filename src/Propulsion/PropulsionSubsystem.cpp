...
PropulsionSubsystem::PropulsionSubsystem(std::shared_ptr<Gtk::Application> &app)
    : Subsystem{"Propulsion", app}
{
}
...
void PropulsionSubsystem::Create()
{
    ...
    std::vector<std::function<void()>> buttonActions = {
        [this, &returnsig]() { // "Create Engine/Thruster Pair"
            ...
            if ((checkSelect(selectedEngine) == true) && (checkSelect(selectedThruster) == true))
            { // Engine/Thruster select check
        ...
        [this]() { // "Delete Tank"
            if ((checkSelect(selectedTank) == true))
            { // Check confirming tank is selected vefore calling confirm Delete Tank
                auto dialog = Gtk::make_managed<Gtk::MessageDialog>(
    ...
    for (int i = 3; i < 5; i++)
    { // Shifts buttons to columns 3 and 4
        for (int j = 1; j < 4; j++)
        { // Lowered buttons to row 1 through 3
