...
BuoyantForcesSubsystem::BuoyantForcesSubsystem(std::shared_ptr<Gtk::Application> &app)
    : Subsystem{"Buoyant Forces", app}
{
}
...
void BuoyantForcesSubsystem::SaveXMLData()
{
    ...
    try
    {
        ...
        if (m_buoyantforces.isBuoyantForcesActive())
        {
            ...
            SetNodesFromWidgets(gascellNode, m_gascell);
            ...
            if (m_gascell.getBallonetCount() > 0)
            {
                for (int i = 0; i < m_ballonets.size(); i++)
                {
                    ...
                    SetNodesFromWidgets(ballonetNodes.back(), m_ballonets[i]);
                    gascellNode.AddChild(ballonetNodes.back());
                }
            }
...
void BuoyantForcesSubsystem::UpdateData()
{
    ...
    for (const auto &[key, dropdownptr] : m_dropdowns)
    {
        ...
        if (selectedIndex >= 0)
        {
            ...
            if (stringlist)
            {
                ...
                if (key.back() == '0')
                {
                    if (fixedKey == "Gas Type")
                        m_gascell.setGasType(static_cast<Component::GasType>(selectedIndex));
                    if (fixedKey == "Location")
                        m_gascell.setLocationUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "Dimensions")
                        m_gascell.setDimensionsUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "x Dim")
                        m_gascell.setDimensionsShape(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "Max Overpressure")
                        m_gascell.setPressureUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "Valve Coefficient")
                        m_gascell.setValveCoefficientUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "# of Ballonets")
                        m_gascell.setBallonetCount(std::stoi(stringlist->get_string(selectedIndex)));
                }
                else
                {
                    int ballonetIndex = key.back() - '0' - 1;

                    if (fixedKey == "Gas Type")
                        m_ballonets[ballonetIndex].setGasType(static_cast<Component::GasType>(selectedIndex));
                    if (fixedKey == "Location")
                        m_ballonets[ballonetIndex].setLocationUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "Dimensions")
                        m_ballonets[ballonetIndex].setDimensionsUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "x Dim")
                        m_ballonets[ballonetIndex].setDimensionsShape(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "Max Overpressure")
                        m_ballonets[ballonetIndex].setPressureUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                    if (fixedKey == "Valve Coefficient")
                        m_ballonets[ballonetIndex].setValveCoefficientUnit(stringToUnit(stringlist->get_string(selectedIndex)));
                }
            }
            else
            {
    ...
    for (const auto &[key, entryptr] : m_entries)
    {
        ...
        if (currentText.empty())
            ...
            textAsDouble = std::stod(currentText);

        std::string fixedKey = key.substr(0, key.size() - 2);
        if (key.back() == '0')
        {
            if (fixedKey == "x Loc")
                m_gascell.setXLocation(textAsDouble);
            if (fixedKey == "y Loc")
                m_gascell.setYLocation(textAsDouble);
            if (fixedKey == "z Loc")
                m_gascell.setZLocation(textAsDouble);
            if (fixedKey == "x Dim")
                m_gascell.setXDimension(textAsDouble);
            if (fixedKey == "y Dim")
                m_gascell.setYDimension(textAsDouble);
            if (fixedKey == "z Dim")
                m_gascell.setZDimension(textAsDouble);
            if (fixedKey == "Max Overpressure")
                m_gascell.setOverpressure(textAsDouble);
            if (fixedKey == "Valve Coefficient")
                m_gascell.setValveCoefficient(textAsDouble);
            if (fixedKey == "Fullness")
                m_gascell.setInitialFullness(textAsDouble);
        }
        else
        {
            int ballonetIndex = key.back() - '0' - 1;
            ...
            if (fixedKey == "x Loc")
                m_ballonets[ballonetIndex].setXLocation(textAsDouble);
            if (fixedKey == "y Loc")
                m_ballonets[ballonetIndex].setYLocation(textAsDouble);
            if (fixedKey == "z Loc")
                m_ballonets[ballonetIndex].setZLocation(textAsDouble);
            if (fixedKey == "x Dim")
                m_ballonets[ballonetIndex].setXDimension(textAsDouble);
            if (fixedKey == "y Dim")
                m_ballonets[ballonetIndex].setYDimension(textAsDouble);
            if (fixedKey == "z Dim")
                m_ballonets[ballonetIndex].setZDimension(textAsDouble);
            if (fixedKey == "Max Overpressure")
                m_ballonets[ballonetIndex].setOverpressure(textAsDouble);
            if (fixedKey == "Valve Coefficient")
                m_ballonets[ballonetIndex].setValveCoefficient(textAsDouble);
            if (fixedKey == "Fullness")
                m_ballonets[ballonetIndex].setInitialFullness(textAsDouble);
            if (fixedKey == "Blower Input")
                m_ballonets[ballonetIndex].setBlowerValue(textAsDouble);
        }
    }
}

void BuoyantForcesSubsystem::SetNodesFromWidgets(JSBEdit::XMLNode &parent, Component &component)
{ // pass in either gas_cell or ballonet node
    ...
    JSBEdit::XMLNode maxoverpressureNode{*xmlptr(), "<max_overpressure></max_overpressure>"};
    ...
    if (parent.GetName() == "gas_cell")
    { // use m_gascell to access data
        // All Attributes needed
        AttributeKV gascellAttr{"type", component.gasTypeToString(component.getGasType())};
        AttributeKV locationAttr{"unit", component.unitToString(component.getLocationUnit())};
        AttributeKV dimensionAttr{"unit", component.unitToString(component.getDimensionsUnit())};
        AttributeKV overpressureAttr{"unit", component.unitToString(component.getPressureUnit())};
        AttributeKV valvecoefficientAttr{"unit", component.unitToString(component.getValveCoefficientUnit())};
        ...
        xlocationNode.SetText(std::to_string(component.getXLocation()));
        ylocationNode.SetText(std::to_string(component.getYLocation()));
        zlocationNode.SetText(std::to_string(component.getZLocation()));
        xdimensionNode.SetText(std::to_string(component.getXDimension()));
        ydimensionNode.SetText(std::to_string(component.getYDimension()));
        zdimensionNode.SetText(std::to_string(component.getZDimension()));
        maxoverpressureNode.SetText(std::to_string(component.getOverpressure()));
        valvecoefficientNode.SetText(std::to_string(component.getValveCoefficient()));
        fullnessNode.SetText(std::to_string(component.getInitialFullness()));
    ...
    { // use m_ballonets vector to access data
        // All Attributes needed
        AttributeKV ballonetAttr{"type", component.gasTypeToString(component.getGasType())};
        AttributeKV locationAttr{"unit", component.unitToString(component.getLocationUnit())};
        AttributeKV dimensionAttr{"unit", component.unitToString(component.getDimensionsUnit())};
        AttributeKV overpressureAttr{"unit", component.unitToString(component.getPressureUnit())};
        AttributeKV valvecoefficientAttr{"unit", component.unitToString(component.getValveCoefficientUnit())};
...
void BuoyantForcesSubsystem::AddEntry(Gtk::Grid &p_grid, std::string label, bool hasDDMenu)
{
    ...
    if (hasDDMenu)
    {
        ...
        if (label.find("Dim") != std::string::npos)
        {
            AddDropDown(p_grid, label, 3, m_shapeStringList);
        }
        if (label == "Max Overpressure")
        {
            AddDropDown(p_grid, label, 3, m_pressureStringList);
        }
        if (label == "Valve Coefficient")
        {
            AddDropDown(p_grid, label, 3, m_valveStringList);
        }
        if (label == "Blower Input")
        {
            AddDropDown(p_grid, label, 3, m_blowerStringList);
        }
    }
    else
    {
...
Component::Unit BuoyantForcesSubsystem::stringToUnit(const std::string &unit_string) const
{
    ...
    if (unit_string == "width")
    {
        UnitType = Component::Unit::WIDTH;
    }
    if (unit_string == "radius")
    {
        UnitType = Component::Unit::RADIUS;
    }

    if (unit_string == "PA")
    {
        UnitType = Component::Unit::PA;
    }
    if (unit_string == "PSI")
    {
        UnitType = Component::Unit::PSI;
    }

    if (unit_string == "M")
    {
        UnitType = Component::Unit::M;
    }
    if (unit_string == "IN")
    {
        UnitType = Component::Unit::IN;
    }

    if (unit_string == "FT4*SEC/SLUG")
    {
        UnitType = Component::Unit::FT4_SEC_SLUG;
    }
    if (unit_string == "M4*SEC/KG")
    {
        UnitType = Component::Unit::M4_SEC_KG;
    }

    if (unit_string == "lbs ft / sec")
    {
        UnitType = Component::Unit::LBS_FT_SEC;
    }
    if (unit_string == "lb ft / (sec R)")
    {
        UnitType = Component::Unit::LB_FT_SEC_R;
    }

    if (unit_string == "ft^3 / sec")
    {
        UnitType = Component::Unit::FT3_SEC;
    }
